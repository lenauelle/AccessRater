...
- (void)mapView:(MKMapView *)mapView annotationView:(MKAnnotationView *)view calloutAccessoryControlTapped:(UIControl *)control
{
    ...
    currentPin.coordinate = view.annotation.coordinate;
    NSString *addr=view.annotation.title;
    
    NSLog(@" %@",addr);
    
  //  PFGeoPoint *cgeoPoint = [PFGeoPoint geoPointWithLatitude:currentPin.coordinate.latitude longitude:currentPin.coordinate.longitude];
    PFQuery *query =[PFQuery queryWithClassName:@"Location"];
    [query whereKey:@"info" equalTo:addr];
    [query getFirstObjectInBackgroundWithBlock:^(PFObject *object, NSError *error) {
        if (!object) {
            NSLog(@"The getFirstObject request failed.");
             NSLog(@" this is DetaillocationExists");
            [self performSegueWithIdentifier:@"Details" sender:view];
        } else {
            // The find succeeded.
            NSLog(@"Successfully retrieved the object.");
            NSLog(@" this is locationExists");
            [self performSegueWithIdentifier:@"locationExists" sender:view];
        }
   }];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"Configure"]) {
        ...
        // WMConfigurationViewController *controller = segue.destinationViewController;
    }else if ([segue.identifier isEqualToString:@"locationExists"]) {
        
        UITabBarController *newvc = [segue destinationViewController];
        MKPointAnnotation *currentPin = [[MKPointAnnotation alloc] init];
        currentPin = [sender annotation];
        NSLog(@" %f",currentPin.coordinate.latitude);
        NSString *addrinfo = currentPin.title;
        PFQuery *aquery =[PFQuery queryWithClassName:@"Location"];
        [aquery whereKey:@"info" equalTo:addrinfo];
        [aquery findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
            if (!error) {
                // NSLog(@"there are %d items",objects.count);
                for (int x =0; x<objects.count; x++) {
                    
                        NSNumber * locID = objects[x][@"PlaceID"];
